...
# Grafik 1: Aufteilung der AHV-Abstimmungen in Teilgebiete 
...
Data Frame mit AHV-Abstimmungen im Bereich Finanzierung erstellen (diese elf, welche wir gemäss unserer Recherche bei der Grafik 1 dem Oberthema Finanzierung zugeteilt haben )
```{r}
...
# Grafik 8: Wähleranteil untersuchen
...
# Grafik 11: Analyse der Haltung des Parlaments

Plot erstellen: Abstimmungen im Bereich AHV, Medien, Militär in Verbindung mit Finanzierung und ob diese angenommen oder abgelehnt wurden und die Position des Parlaments
```{r}
ggplot(finanzierungen_gekuerzt) +
...
# Grafik 13: Wie der Tenor der Medien gegenüber den Initiativen war
```{r}
...
# Grafik 14: Anzahl befürwortende Inserate
```{r}
...
# Grafik 15: Anzahl ablehnende Inserate im Vergleich zum Ergebnis der Initiativen
```{r}
